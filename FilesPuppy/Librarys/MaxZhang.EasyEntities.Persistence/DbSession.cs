    ...
    public class DbSession : IDisposable
    {
       ...
       /// <returns>返回第一行第一列的值</returns>
        ...
        public void Insert<TModel>(Expression<Func<TModel,object>> objExp) where TModel : DbObject
        {
            ...
            FieldsFormater format = new FieldsFormater(Provider.ParamPrefix);
            format.Visit(objExp);
        ...
        public IOperatorWhere<TModel> Remove<TModel>() where TModel : DbObject
        {
            var tableName = DbMetaDataManager.GetTableName(typeof(TModel));
            var removeFieldName = TableTemplateMethod.GetRemoveAbleFieldName(typeof(TModel));
            string template = string.Format("Update {0} SET {1} Where 1=1", tableName, removeFieldName + "=1");
       ...
       /// <summary>
        ...
        private static Dictionary<string, SqlMapCommand> sqlMaps = new Dictionary<string, SqlMapCommand>();
